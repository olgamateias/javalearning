...
//		}
	 ...
	 * 01-23-45-67-89-AB).
	 * 
	 * Your task is to check by given string inputString whether it corresponds to MAC-48 address or
	 * not.
	 * 
	 * Example
	 * 
	...
	boolean isMAC48Address(String inputString) {
		...
		for (int i = 0; i < array.length; i++) {
			if (array[i].length() != 2) {
				...
				for (int x = 0; x < array[i].length(); x++) {
					if ((array[i].charAt(x) < 48 || array[i].charAt(x) > 57) && (array[i].charAt(x) < 65 || array[i].charAt(x) > 70)) {
						System.out.println("false");
						return false;
					}
				}
